...
def run_demucs(mp3_path: Path, model: str = DEFAULT_DEMUCS_MODEL):
    # Reuse step2 device selection (cuda|mps|cpu) so CLI helpers match pipeline behavior.
    from scripts.step2_split import _resolve_demucs_device

    device = _resolve_demucs_device()
    cmd = ["demucs", "-n", "htdemucs", "--shifts", "1", "--overlap", "0.10", "-d", device, "-o", str(PATHS["separated"]), str(mp3_path)]

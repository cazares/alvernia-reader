...
test("Static app config keeps Android package and EAS project id", () => {
  ...
  assert.equal(appConfig?.expo?.android?.package, "com.cazares.mixterious");
...
test("Resolved Expo config preserves release identity and versioning", () => {
  ...
  assert.equal(expoConfig?.android?.package, "com.cazares.mixterious");
  assert.equal(expoConfig?.android?.versionCode, Number(version.buildNumber));
  assert.equal(expoConfig?.ios?.bundleIdentifier, "com.cazares.mixterious");
...
test("Resolved Expo config enforces plugin and OTA guardrails", () => {
  ...
  assert.equal(expoConfig?.updates?.enabled, true);
  assert.equal(
    expoConfig?.updates?.url,
    "https://u.expo.dev/8f4aeff3-940f-4ec2-b82d-89b430f5c8be"
  );
  assert.equal(expoConfig?.extra?.ota?.channel, "production");
  assert.equal(expoConfig?.extra?.ota?.enabled, true);
  assert.equal(expoConfig?.extra?.ota?.autoCheckOnForeground, true);
});

...
test("web app script supports first-page startup, song-based navigation numberpad input, and simple fullscreen", () => {
  ...
  assert.match(source, /getAdjacentSongPages/);
  assert.match(source, /prefetchSongPage/);
  ...
  assert.match(source, /fetch\(pageFileName\(pageNumber\), \{ cache: "force-cache" \}\)/);
  assert.match(source, /serviceWorker\.register\("\/sw\.js"\)/);
  assert.doesNotMatch(source, /preloadPage/);
  assert.match(source, /scheduleLoadingIndicator/);
  assert.match(source, /hideLoadingIndicator/);

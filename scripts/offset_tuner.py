...
def tune_offset(
    *,
    ...
    auto_path = timings_dir / f"{slug}.offset.auto"

    # Offset precedence: manual > auto > base_offset
    # - offset.auto is auto-estimated (Whisper)
    # - offset is MANUAL override (replaces auto, doesn't add)
    auto = None
    try:
        if auto_path.exists():
            raw = auto_path.read_text(encoding="utf-8").strip()
            auto = float(raw) if raw else 0.0
    except Exception:
        auto = None
    if auto is None:
        auto = float(base_offset)

    existing_manual = _read_offset_file(offset_path)
    manual = float(existing_manual) if existing_manual is not None else 0.0

    # User tunes the EFFECTIVE offset (manual if set, else auto)
    # When saved, it goes to .offset (manual), completely replacing auto
    offset = manual if abs(manual) > 1e-6 else auto
        ...
        if choice in ("1", "e", "earlier"):
            ...
            while True:
                ...
                if result == "LATER":
                    ...
                    continue  # immediate replay
                if result == "LOCK":
                    _write_offset_file(offset_path, float(offset) - float(auto))
